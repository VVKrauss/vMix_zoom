-- Presence: make online window 15s usable by updating last_active_at more often.
-- Client считает онлайн по user_presence_public.last_active_at окном 15s.
-- Поэтому pulse должен обновлять last_active_at чаще, чем раз в 40 секунд.
...
-- Align peek helper with 15s window (optional, but avoids mismatch with mirror-based UI).
...
begin
  ...
  if v_me = v_target then
    v_online :=
      ...
      and r.last_active_at > (now() - interval '15 seconds')
      and (
