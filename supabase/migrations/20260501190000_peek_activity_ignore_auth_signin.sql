-- Fix: «последняя активность» не должна обновляться от фоновых auth событий
-- (например, refresh сессии / сервис-воркер при получении сообщения).
-- Учитываем только явную активность на сайте (last_active_at) и явный вход (last_login_at).
...
begin
  ...
  from unnest(array[r.last_active_at, r.last_login_at]) as t(x)
  where x is not null;
