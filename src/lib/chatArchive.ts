...
export async function listRoomChatLastSenders(
  conversationIds: string[],
): Promise<{ data: Record<string, RoomChatLastSender> | null; error: string | null }> {
  const ids = Array.from(new Set(conversationIds.map((id) => id.trim()).filter(Boolean)))
  if (ids.length === 0) return { data: {}, error: null }

  const { data: messageRows, error: messageError } = await supabase
    .from('chat_messages')
    .select('conversation_id, sender_user_id, sender_name_snapshot, created_at')
    .in('conversation_id', ids)
    .order('created_at', { ascending: false })

  if (messageError) return { data: null, error: messageError.message }

  const latestByConversation = new Map<string, { senderUserId: string | null; senderNameSnapshot: string; createdAt: string }>()
  ...
  const senderIds = Array.from(
    new Set(
      Array.from(latestByConversation.values())
        .map((item) => item.senderUserId?.trim() ?? '')
        .filter(Boolean),
    ),
  )
  ...
  const mapped: Record<string, RoomChatLastSender> = {}
  for (const [conversationId, item] of latestByConversation.entries()) {
    mapped[conversationId] = {
      conversationId,
      senderUserId: item.senderUserId,
      senderNameSnapshot: item.senderNameSnapshot,
      avatarUrl: item.senderUserId ? (avatarMap.get(item.senderUserId) ?? null) : null,
