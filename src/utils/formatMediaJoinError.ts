  ...
  if (err instanceof DOMException) {
    switch (err.name) {
      case 'NotAllowedError':
        return (
          'Нет доступа к микрофону или камере. Разрешите их для этого сайта в настройках страницы ' +
          '(в адресной строке: меню сайта / замок / «Аа» — зависит от браузера; на iPhone шаги похожи во всех браузерах) ' +
          'либо выключите микрофон и камеру ниже и войдите без них.'
        )
      case 'NotFoundError':
        return 'Микрофон или камера не найдены. Подключите устройство или войдите с выключенными микрофоном и камерой.'
      case 'NotReadableError':
        return (
          'Не удалось захватить камеру или микрофон. На iPhone это бывает даже при включённом доступе в настройках ' +
          '(все браузеры используют WebKit). Подождите несколько секунд, перезагрузите страницу, закройте другие вкладки с камерой ' +
          'или войдите без микрофона и камеры и включите их уже после входа.'
        )
